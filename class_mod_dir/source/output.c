...
static size_t my_fwrite(void *ptr,size_t size,size_t nmemb,FILE *fil)
{
  size_t nwritten;

  if((nwritten=fwrite(ptr,size,nmemb,fil))!=nmemb) {
...
static void write_columntitle(FILE *f,char *str,int condition,int *colnum)
{
  if(condition==_TRUE_) {
    fprintf(f,"[%d]-%s, ",*colnum,str);
    (*colnum)++;
  }
}
...
int output_init(
  ...
  if (pop->write_sigma == _TRUE_){
    
    class_call(output_sigma(pba,pth,ppt,ppm,psp,pop),
               pop->error_message,
	       pop->error_message);
...
int output_open_cl_file(
  ...
  if(pop->write_binary==1) {
    class_open(*clfile,filename,"wb",pop->error_message);
    my_fwrite(&(lmax),sizeof(int),1,*clfile);
    my_fwrite(&(psp->d_size_nc),sizeof(int),1,*clfile);
    my_fwrite(&(psp->d_size_wl),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_tt),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_ee),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_te),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_bb),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_pp),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_tp),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_ep),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_td),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_tl),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_pd),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_pl),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_dd),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_dl),sizeof(int),1,*clfile);
    my_fwrite(&(psp->has_ll),sizeof(int),1,*clfile);
  }
  else {
    ...
    if (pop->write_header == _TRUE_) {
      ...
      fprintf(*clfile,"#\n");
      fprintf(*clfile,"# -> if you prefer output in CAMB/HealPix/LensPix units/order, set 'format' "
	      "to 'camb' in input file\n");
      fprintf(*clfile,"# -> if you don't want to see such a header, set 'headers' to 'no' in input file\n");
      fprintf(*clfile,"#\n");
      fprintf(*clfile,"# [0]-l ");      colnum++;

      write_columntitle(*clfile,"TT",psp->has_tt,&colnum);
      write_columntitle(*clfile,"EE",psp->has_ee,&colnum);
      write_columntitle(*clfile,"TE",psp->has_te,&colnum);
      write_columntitle(*clfile,"BB",psp->has_bb,&colnum);
      write_columntitle(*clfile,"PP",psp->has_pp,&colnum);
      write_columntitle(*clfile,"TP",psp->has_tp,&colnum);
      write_columntitle(*clfile,"EP",psp->has_ep,&colnum);

      /** Rest of the entries are independent of format type */
      if (psp->has_td == _TRUE_){
	for (index_d1=0; index_d1<psp->d_size_nc; index_d1++){
	  sprintf(tmp,"td%d",index_d1+1);
	  write_columntitle(*clfile,tmp,_TRUE_,&colnum);
	}
      }
      if (psp->has_tl == _TRUE_){
	for (index_d1=0; index_d1<psp->d_size_wl; index_d1++){
	  sprintf(tmp,"tl%d",index_d1+1);
	  write_columntitle(*clfile,tmp,_TRUE_,&colnum);
	}
      }
      if (psp->has_pd == _TRUE_){
	for (index_d1=0; index_d1<psp->d_size_nc; index_d1++){
	  sprintf(tmp,"pd%d",index_d1+1);
	  write_columntitle(*clfile,tmp,_TRUE_,&colnum);
	}
      }
      if (psp->has_pl == _TRUE_){
	for (index_d1=0; index_d1<psp->d_size_wl; index_d1++){
	  sprintf(tmp,"pl%d",index_d1+1);
	  write_columntitle(*clfile,tmp,_TRUE_,&colnum);
	}
      }
	...
	for (index_d1=0; index_d1<psp->d_size_nc; index_d1++){
	  for (index_d2=index_d1; index_d2<psp->d_size_nc; index_d2++){
	    sprintf(tmp,"d%dd%d",index_d1+1,index_d2+1);
	    write_columntitle(*clfile,tmp,_TRUE_,&colnum);
	  }
	}
      }
	...
	for (index_d1=0; index_d1<psp->d_size_nc; index_d1++){
	  for (index_d2=0; index_d2<psp->d_size_wl; index_d2++){
	    sprintf(tmp,"d%dl%d",index_d1+1,index_d2+1);
	    write_columntitle(*clfile,tmp,_TRUE_,&colnum);
	  }
	}
      }
	...
	for (index_d1=0; index_d1<psp->d_size_wl; index_d1++){
	  for (index_d2=index_d1; index_d2<psp->d_size_wl; index_d2++){
	    sprintf(tmp,"l%dl%d",index_d1+1,index_d2+1);
	    write_columntitle(*clfile,tmp,_TRUE_,&colnum);
	  }
	}
      }
...
int output_one_line_of_cl(
  ...
  factor = l*(l+1)/2./_PI_;
  for(index_d1=0;index_d1<ct_size;index_d1++)
    cl[index_d1]*=factor;

  if(pop->write_binary==1) {
    if(psp->has_tt==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_tt]),sizeof(double),1,clfile);
    if(psp->has_ee==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_ee]),sizeof(double),1,clfile);
    if(psp->has_te==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_te]),sizeof(double),1,clfile);
    if(psp->has_bb==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_bb]),sizeof(double),1,clfile);
    if(psp->has_pp==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_pp]),sizeof(double),1,clfile);
    if(psp->has_tp==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_tp]),sizeof(double),1,clfile);
    if(psp->has_ep==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_ep]),sizeof(double),1,clfile);

    if(psp->has_td==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_td]),sizeof(double),psp->d_size_nc,clfile);
    if(psp->has_tl==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_tl]),sizeof(double),psp->d_size_wl,clfile);
    if(psp->has_pd==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_pd]),sizeof(double),psp->d_size_nc,clfile);
    if(psp->has_pl==_TRUE_)
      my_fwrite(&(cl[psp->index_ct_pl]),sizeof(double),psp->d_size_wl,clfile);

    if(psp->has_dd==_TRUE_) {
      my_fwrite(&(cl[psp->index_ct_dd]),sizeof(double),
		(psp->d_size_nc*(psp->d_size_nc+1))/2,clfile);
    }
    if(psp->has_dl==_TRUE_) {
      my_fwrite(&(cl[psp->index_ct_dl]),sizeof(double),
		psp->d_size_nc*psp->d_size_wl,clfile);
    }
    if(psp->has_ll==_TRUE_) {
      my_fwrite(&(cl[psp->index_ct_ll]),sizeof(double),
		(psp->d_size_wl*(psp->d_size_wl+1))/2,clfile);
    }
  }
  else {
    fprintf(clfile," ");
    ...
    if(psp->has_tt==_TRUE_)
      fprintf(clfile,"%lE ",cl[psp->index_ct_tt]);
    if(psp->has_ee==_TRUE_)
      fprintf(clfile,"%lE ",cl[psp->index_ct_ee]);
    if(psp->has_te==_TRUE_)
      fprintf(clfile,"%lE ",cl[psp->index_ct_te]);
    if(psp->has_bb==_TRUE_)
      fprintf(clfile,"%lE ",cl[psp->index_ct_bb]);
    if(psp->has_pp==_TRUE_)
      fprintf(clfile,"%lE ",cl[psp->index_ct_pp]);
    if(psp->has_tp==_TRUE_)
      fprintf(clfile,"%lE ",cl[psp->index_ct_tp]);
    if(psp->has_ep==_TRUE_)
      fprintf(clfile,"%lE ",cl[psp->index_ct_ep]);
    if (psp->has_td == _TRUE_) {
      for(index_d1=0;index_d1<psp->d_size_nc;index_d1++)
	fprintf(clfile,"%lE ",cl[psp->index_ct_td+index_d1]);
    }
    if (psp->has_tl == _TRUE_) {
      for(index_d1=0;index_d1<psp->d_size_wl;index_d1++)
	fprintf(clfile,"%lE ",cl[psp->index_ct_tl+index_d1]);
    }
    if (psp->has_pd == _TRUE_) {
      for(index_d1=0;index_d1<psp->d_size_nc;index_d1++)
	fprintf(clfile,"%lE ",cl[psp->index_ct_pd+index_d1]);
    }
    if (psp->has_pl == _TRUE_) {
      for(index_d1=0;index_d1<psp->d_size_wl;index_d1++)
	fprintf(clfile,"%lE ",cl[psp->index_ct_pl+index_d1]);
    }
    if (psp->has_dd == _TRUE_) {
      int index_cl=0;
      for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
	for(index_d2=index_d1;index_d2<psp->d_size_nc;index_d2++) {
	  fprintf(clfile,"%lE ",cl[psp->index_ct_dd+index_cl]);
	  index_cl++;
	}
      }
    }
    if (psp->has_dl == _TRUE_) {
      ...
      for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
	for(index_d2=0;index_d2<psp->d_size_wl;index_d2++) {
	  fprintf(clfile,"%lE ",cl[psp->index_ct_dl+index_cl]);
	  index_cl++;
	}
      }
    }
    if (psp->has_ll == _TRUE_) {
      ...
      for(index_d1=0;index_d1<psp->d_size_wl;index_d1++) {
	for(index_d2=index_d1;index_d2<psp->d_size_wl;index_d2++) {
	  fprintf(clfile,"%lE ",cl[psp->index_ct_ll+index_cl]);
	  index_cl++;
	}
      }
    }
...
/**
 * This routine writes sigma at different scales and redshfits
 */
...
int output_sigma(
  ...
  FILE * sigmafile;
  FileName file_name;
  double sigma_8, sigma_100, sigma_rs, sigma_r;
  double k_wish, k_used, diff;
  double z, tau;
  double * f_cdm;
  int i, i_k ;
  
  sprintf(file_name,"%s%s",pop->root,"sigma.dat");
  class_open(sigmafile,file_name,"w",pop->error_message);

  //scale at which sigma and pk will be computed
  k_wish = 1./pop->r_sigma;
  
  //find the value of k closest to k_wish = 1/r
  diff = 1e10;
  for (i = 0; i <= ppt->k_size[psp->index_md_scalars]; i ++){
     if (fabs(ppt->k[psp->index_md_scalars][i] - k_wish)<diff){
       k_used = ppt->k[psp->index_md_scalars][i];
       diff = fabs(k_used-k_wish);
       i_k = i;
     } 
  }
  printf("k_wish = %e, k_used = %e, i_k =%i\n",k_wish,k_used,i_k);
  
  if (pop->write_header == _TRUE_) {
    fprintf(sigmafile,"# Table of growth factor f = dlog(delta_cdm)/dlog(a) and sigma_r = sigma(r/h,z) (units are in Mpc) \n");
    fprintf(sigmafile,"# Values used r = %e,  k_wish = %e = 1/r, k_used = %e (closest one), r_s(z_d)=%g, \n",
	    pop->r_sigma,k_wish,k_used, pth->rs_d);
    fprintf(sigmafile,"# z, f sigma_r, f, sigma_r, sigma_8, sigma_100, sigma_r_s \n");
  }
  
  //aovid trouble
  if (psp->z_max_pk < pop->z_sigma_end)
    pop->z_sigma_end = psp->z_max_pk;
  
  
  //allocate vectors containing sources as a function of k for tau
  class_alloc(f_cdm,sizeof(double)*ppt->k_size[psp->index_md_scalars],pop->error_message);
  
  // computing sigma may take a while...
  //  for (z =0.; z<=pop->z_sigma_end; z+= 1.1*z + 0.1){
  for (z =0.; z<=pop->z_sigma_end; z+=0.01){
    
    class_call(background_tau_of_z(pba,z,&tau),
               pba->error_message,
               pop->error_message);
    
    // index_ic = 0 should be for adiabatic //TODO: substitute by sum of all IC
    class_call(perturb_sources_at_tau(ppt,psp->index_md_scalars,
				      0,
				      ppt->index_tp_f_cdm,
				      tau,
				      f_cdm),
	       ppt->error_message, psp->error_message);
    
    class_call(spectra_sigma(pba,ppm,psp,pop->r_sigma/pba->h,z,&(sigma_r)),
               psp->error_message,
               pop->error_message);
    class_call(spectra_sigma(pba,ppm,psp,8./pba->h,z,&(sigma_8)),
               psp->error_message,
               pop->error_message);
    class_call(spectra_sigma(pba,ppm,psp,100./pba->h,z,&(sigma_100)),
               psp->error_message,
               pop->error_message);
    class_call(spectra_sigma(pba,ppm,psp,pba->rs_d,z,&(sigma_rs)),
               psp->error_message,
               pop->error_message);
    fprintf(sigmafile,"%e  %e  %e  %e  %e  %e  %e  \n",z, f_cdm[i_k]*sigma_r, f_cdm[i_k], sigma_r,sigma_8,sigma_100,sigma_rs);
