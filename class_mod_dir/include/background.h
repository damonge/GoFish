...
enum gravity_model {einstein, quintessence, galileon, brans_dicke, cccg_exp, cccg_pow, abp_grav, galileon_param, propto_omega, constant_alphas, propto_scale, threshold_alphas, inv_threshold_alphas, planck_linear, planck_exponential,propto_omega_b2,series_omega}; //write here the different models

// initial conditions for the perturbations
enum pert_initial_conditions {single_clock, zero, kin_only};

// enum gravity_model_subclass {quint_exp, cccg_exp, cccg_pow}; //write here model subclasses

enum expansion_model {lcdm, wede, wmr, lede, cpl}; //parameterized expansion, only for non-self consistent Horndeski theories

// method to solve the Friedmann equation
// so far only the cases in the first line have been implemented
enum hubble_method {h_last, cubic_3_exact, cubic_3_taylor, cubic_1_exact, 
                    cubic_3_taylor_p, cubic_3_taylor_m, cubic_1_exact_p, cubic_1_exact_m};
...
struct background
{
  ...
  int scf_tuning_index;     /**< index in scf_parameters used for tuning (Omega_de) */
...
//   enum gravity_model_subclass gravity_submodel_smg; /** Horndeski model */
  ...
  int friedmann_branch_smg; /* choice of branch: 1,2,3 gives smallest to largest possible values of H, 0 gives closest to sqrt(rho) */
  ...
  double hubble_cubic_taylor_tol_smg; /**< threshold to consider the full cubic Friedmann equation or an approxmation */
  double hubble_cubic_discrim_tol_smg; /**< threshold to consider the discriminant as positive */
  double hubble_continuity_tol_smg; /**< threshold to avoid discontinuities in the Friedmann equation */
  ...
  int scalar_eq_order_smg; /**< order of the scalar field equation: can be second (2), first (1) or algebraic (0) */
  ...
  double z_ref_smg; /**< reference z to specify IC in some models */
  ...
  double * parameters_3_smg;  /**< list of auxiliary parameters describing the modified gravity model (for series) */
  double * parameters_4_smg;  /**< list of auxiliary parameters describing the modified gravity model (for series) */
  double * parameters_5_smg;  /**< list of auxiliary parameters describing the modified gravity model (for series) */
  ...
  double H_last_smg; /** parameter to write the last value of H obtained in the integration, and so being able to pick up the right value from the integrator */
  double H_first_smg; /** parameter to write the initial value of H. Necessary to pick up from the correct value when writing the background table*/
  
  enum hubble_method H_method_last_smg;
  enum hubble_method H_method_new_smg;
  ...
  int index_bg_alpha_4_smg; /**< second order EFT quartic function */
  int index_bg_alpha_5_smg; /**< second order EFT quintic function */
  int index_bg_alpha_4_prime_smg; /**< quartic function deriv wrt conf time */
  int index_bg_alpha_5_prime_smg; /**< quintic function deriv wrt conf time */  
  ...
  int index_bg_G_phi_smg; /** QS potentials */
  int index_bg_G_psi_smg;
  int index_bg_G_field_smg;     
  int index_bg_G_eff_0_smg;     
  int index_bg_G_eff_2_smg;     
  int index_bg_eta_0_smg;     
  int index_bg_eta_2_smg;     
