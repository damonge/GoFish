  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
	...
	if(ppt->has_intrinsic_alignment)
      ...
      if ((ppt->has_pk_matter == _TRUE_) || (ppt->has_nl_corrections_based_on_delta_m)) {
        ppt->has_lss = _TRUE_;
        ppt->has_source_delta_m = _TRUE_;
	ppt->has_source_f_cdm = _TRUE_; //growth factor, used for f sigma_r, etc...
      }
	...
	if (pba->has_smg == _TRUE_)
      ...
      class_define_index(ppt->index_tp_f_cdm,      ppt->has_source_f_cdm,     index_type,1);
	...
	my_abort(1,"CLASST version only supports a single initial condition type\n");
#endif //_CLASST
    }
...
int perturb_workspace_init(
  ...
  if (_scalars_) {
    ...
    if (pba->has_smg == _TRUE_) {
      ppw->approx[ppw->index_ap_smgqs]=(int)smgqs_on_extreme;
    }
  }
...
int perturb_solve(
  ...
  /* number of intervals at which the smgqs switches */
  int number_switches_accepted_smgqs = 4;
  double * interval_number_smgqs;
  ...
  while ((tau_upper - tau_lower)/tau_lower > ppr->tol_tau_approx) {
    ...
    /* Force integration to start at a high redshift. 
     * Important to test smg models with early MG 
     */
    if(ppw->pvecback[pba->index_bg_a] > pba->min_a_pert_smg)
      is_early_enough = _FALSE_;
  ...
  class_alloc(interval_number_smgqs,(number_switches_accepted_smgqs + 1)*sizeof(double),ppt->error_message);

  if (pba->has_smg == _TRUE_) {
    class_call(perturb_find_smgqs_intervals(ppr,
                                            pba,
				            ppt,
                                            k,
                                            tau,
                                            ppt->tau_sampling[tau_actual_size-1],
                                            number_switches_accepted_smgqs,
                                            interval_number_smgqs,
                                            ppw),
               ppt->error_message,
               ppt->error_message);
  }
...
//     printf("solve: smgqs = %i (off = %i, %i / on = %i, %i, on_xtr = %i) \n",ppw->approx[ppw->index_ap_smgqs],
// 	       (int)smgqs_off,(int)smgqs_off_again,(int)smgqs_on,(int)smgqs_on_again,(int)smgqs_on_extreme);
//     printf("for tau in [%e, %e] \n",interval_limit[index_interval],interval_limit[index_interval+1]);
...
int perturb_find_approximation_number(
                                      ...
                                      double * interval_number_smgqs,
				      int number_switches_accepted_smgqs
                                      ){
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_scalars_) {
          ...
          if (pba->has_smg == _TRUE_) {
            if ((interval_approx[index_switch-1][ppw->index_ap_smgqs]==(int)smgqs_on_extreme) &&
                (interval_approx[index_switch][ppw->index_ap_smgqs]==(int)smgqs_on)) {
              fprintf(stdout,"Mode k=%e: will switch on smg quasi-static approximation at tau=%e\n",k,interval_limit[index_switch]);
            }
	  }
          if (pba->has_smg == _TRUE_) {
            if ((interval_approx[index_switch-1][ppw->index_ap_smgqs]==(int)smgqs_on) &&
                (interval_approx[index_switch][ppw->index_ap_smgqs]==(int)smgqs_off)) {
              fprintf(stdout,"Mode k=%e: will switch off smg quasi-static approximation at tau=%e\n",k,interval_limit[index_switch]);
            }
	  }
          if (pba->has_smg == _TRUE_) {
            if ((interval_approx[index_switch-1][ppw->index_ap_smgqs]==(int)smgqs_off) &&
                (interval_approx[index_switch][ppw->index_ap_smgqs]==(int)smgqs_on_again)) {
              fprintf(stdout,"Mode k=%e: will switch on_again smg quasi-static approximation at tau=%e\n",k,interval_limit[index_switch]);
            }            
          }
          if (pba->has_smg == _TRUE_) {
            if ((interval_approx[index_switch-1][ppw->index_ap_smgqs]==(int)smgqs_on_again) &&
                (interval_approx[index_switch][ppw->index_ap_smgqs]==(int)smgqs_off_again)) {
              fprintf(stdout,"Mode k=%e: will switch off_again smg quasi-static approximation at tau=%e\n",k,interval_limit[index_switch]);
            }            
          }
        }
...
int perturb_vector_init(
  ...
  if (_scalars_) {
     ...
     */    
    if ((pba->has_smg == _TRUE_) && ((ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off) || (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off_again))){
    ...
    if ((pba->has_smg == _TRUE_) && ((ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on) || (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on_again))){
      class_define_index(ppv->index_pt_vx_smg,_TRUE_,index_pt,1); /* dynamical scalar field perturbation */
    }    
...
//     printf(" %e \t %e \n",k,tau);
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
	...
	if ((pa_old[ppw->index_ap_smgqs] == (int)smgqs_on_extreme) && (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on)){
	  ppv->y[ppv->index_pt_vx_smg] =
	    ppw->pvecmetric[ppw->index_mt_vx_prime_smg];
	}
	else if ((ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off) || (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off_again)) {
	  ppv->y[ppv->index_pt_vx_smg] =
	    ppw->pv->y[ppw->pv->index_pt_vx_smg];
	  ppv->y[ppv->index_pt_vx_prime_smg] =
	    ppw->pvecmetric[ppw->index_mt_vx_prime_smg];
	}
	else if ((ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on) || (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on_again)){
	  ppv->y[ppv->index_pt_vx_smg] =
	    ppw->pv->y[ppw->pv->index_pt_vx_smg];
	}
	if ((pa_old[ppw->index_ap_smgqs] == (int)smgqs_on_extreme) && (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on)){
	  if (ppt->perturbations_verbose>2)
	    fprintf(stdout,"Mode k=%e: switch on quasi-static approximation at tau=%e\n",k,tau);
	}
	if ((pa_old[ppw->index_ap_smgqs] == (int)smgqs_on) && (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off)){
	  if (ppt->perturbations_verbose>2)
	    fprintf(stdout,"Mode k=%e: switch off quasi-static approximation at tau=%e\n",k,tau);
	}
	if ((pa_old[ppw->index_ap_smgqs] == (int)smgqs_off) && (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on_again)){
	  if (ppt->perturbations_verbose>2)
	    fprintf(stdout,"Mode k=%e: switch on_again quasi-static approximation at tau=%e\n",k,tau);
	}
	if ((pa_old[ppw->index_ap_smgqs] == (int)smgqs_on_again) && (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off_again)){
	  if (ppt->perturbations_verbose>2)
	    fprintf(stdout,"Mode k=%e: switch off_again quasi-static approximation at tau=%e\n",k,tau);
	}
      }        
  ...
  if (_scalars_) {
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
       ...
       * 
       * NEW: kineticity only IC: Vx = (k tau)^2
       * from Vx'' = 2 (a H)^2 Vx
       */     
      if (pba->has_smg == _TRUE_) { 
	
// 	printf("IC: smgqs = %i (off = %i, %i / on = %i, %i, on_xtr = %i) \n",ppw->approx[ppw->index_ap_smgqs],
// 	       (int)smgqs_off,(int)smgqs_off_again,(int)smgqs_on,(int)smgqs_on_again,(int)smgqs_on_extreme);
	
	if (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off ) {
	  
	  double dt = -1/(4.*ppw->pvecback[pba->index_bg_H])*ppw->pv->y[ppw->pv->index_pt_delta_g];
...
// 	      printf("k = %e, ktau_three = %e, ktau_two = %e \n",k, ktau_three,ktau_two);
	      if(ppt->perturbations_verbose > 5)
		printf("Kin_only IC for smg: ");
	    }  
	...
	else if (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on) {
	  	 
	ppw->pv->y[ppw->pv->index_pt_vx_smg] = -1/(4.*ppw->pvecback[pba->index_bg_H])*ppw->pv->y[ppw->pv->index_pt_delta_g]*ktau_two; // single_clock IC given with respect to photons (because there are always photons)
	//
	 
	if(ppt->perturbations_verbose > 5)
	    printf("smg initial conditions (QUASI-DYNAMIC!) Vx = %e, \n",ppw->pv->y[ppw->pv->index_pt_vx_smg]); 
	}
      }        
...
int perturb_approximations(
  ...
  double tau_c;
  /* (d) time scale of scalar field oscillations \f$ \tau_{s} = 1/(c_{s}k) \f$ depends on speed of sound */
  double tau_s=0;
  ...
  if (_scalars_) {
    ...
    /* (d) quasi-static modified gravity
     * condition is tau/tau_s = c_s k tau > ppr->smgqs_trigger_tau_over_tau_s
     * indicating that the field is rapidly oscillating
     * if the speed of sound is imaginary, no QS approximation
     */
    ...
    if (pba->has_smg == _TRUE_) {
      ...
      double cs2n = ppw->pvecback[pba->index_bg_cs2num_smg];
      double g1 = ppw->pvecback[pba->index_bg_gamma_1_smg];
      double g2 = ppw->pvecback[pba->index_bg_gamma_2_smg];
      double g3 = ppw->pvecback[pba->index_bg_gamma_3_smg];
      
      double M2cs2 = (cs2n*pow(k,4) + g3*pow(H,2)*pow(k,2)*pow(a,2) + g2*pow(H,4)*pow(a,4))*pow(D*pow(k,2) + g1*pow(H,2)*pow(a,2),-1);
      
      tau_s = tau*H*a/sqrt(M2cs2);
      

      if (interval_number_smgqs[number_switches_accepted_smgqs] == 0.) {
        if (tau > interval_number_smgqs[3]) {
	  ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_off_again;
        }
        else if (tau > interval_number_smgqs[2]) {
	  if (tau/tau_s > ppr->smgqs_trigger_tau_over_tau_s) {
	    ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_on_again;
	  }
	  else {
	    ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_off_again;
	  }
        }
        else if (tau > interval_number_smgqs[1]) {
	  if (tau/tau_s > ppr->smgqs_trigger_tau_over_tau_s) {
	    ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_on_again;
	  }
	  else {
	    ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_off;
	  }
        }
        else if (tau >= interval_number_smgqs[0]) {
	  if (tau/tau_s > ppr->smgqs_trigger_tau_over_tau_s) {
	    ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_on;
	  }
	  else {
	    ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_off;
	  }
        }
        else if (tau/tau_s > ppr->smgqs_extreme_trigger_tau_over_tau_s) {
	  ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_on_extreme;
        }
        else {
	  ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_on;
      ...
      else if (interval_number_smgqs[number_switches_accepted_smgqs] == 1.) {
	if (tau > interval_number_smgqs[3]) {
	  ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_off_again;
	}
	else if (tau > interval_number_smgqs[2]) {
	  ...
	  if (tau/tau_s > ppr->smgqs_extreme_trigger_tau_over_tau_s) {
	    ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_on_again;
	  }
	  else {
	    ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_off;
	...
	ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_off;
      ...
      //NOTE: overwrite the approximation schemes to always be fully dynamical
      ppw->approx[ppw->index_ap_smgqs] = (int)smgqs_off;
    }
  }
...
int perturb_einstein(
  ...
  if (_scalars_) {
	...
	D = ppw->pvecback[pba->index_bg_kinetic_D_smg];
        ...
        if ((ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off) || (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off_again)) {
	  ...
	  class_test(isnan(ppw->pvecmetric[ppw->index_mt_vx_prime_prime_smg]),
        ...
        }//end of ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_off
        else if ((ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on) || (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on_again)) {
	  ...
	  /* scalar field equation */
	  ppw->pvecmetric[ppw->index_mt_vx_prime_smg] = 3./8.*pow(H,-2)*l2*pow(l7,-1)*pow(M2,-1)*ppw->delta_rho + ((-2.) + bra)*9./16.*bra*pow(H,-2)*pow(l7,-1)*pow(M2,-1)*ppw->delta_p + 1./4.*cs2num*pow(H,-2)*pow(k,2)*pow(l7,-1)*y[ppw->pv->index_pt_eta]*pow(a,-2) + (cs2num*pow(k,2) + (-4.)*pow(H,2)*l8*pow(a,2))*(-1.)/4.*pow(H,-1)*pow(l7,-1)*ppw->pvecmetric[ppw->index_mt_vx_smg]*pow(a,-1);

        }//end of ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on
        else if (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on_extreme) {
	  ...
	  ppw->pvecmetric[ppw->index_mt_vx_prime_smg] = 0.;

        }//end of ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on_extreme
        else {
	  printf("scalar field equation: QS approximation mode %i not recognized. should be QS on_extreme, on, off, on_again or off_again\n",ppw->approx[ppw->index_ap_smgqs]);
	  return _FAILURE_;
        }
	...
	if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_on) {
	  ...
	  if (pba->has_ur == _TRUE_) {
        ...
        /* eventually, infer first-order tight-coupling approximation for photon shear, then correct the total shear */
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    /* growth factor for CDM 
     * f = dlog(D)/dlog(a) = delta'/delta = delta'/delta * a/a' */
    if (ppt->has_source_f_cdm == _TRUE_){
     _set_source_(ppt->index_tp_f_cdm) = dy[ppw->pv->index_pt_delta_cdm]/y[ppw->pv->index_pt_delta_cdm] /a_prime_over_a;
    }
  }
...
// 	  printf(" a = %e v_x' = %e, vx'' = %e \n",a,dy[pv->index_pt_vx_smg],dy[pv->index_pt_vx_prime_smg]);

	}
	if ((ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on) || (ppw->approx[ppw->index_ap_smgqs] == (int)smgqs_on_again)) {
...
int perturb_find_smgqs_intervals(
                                 struct precision * ppr,
                                 struct background * pba,
                                 struct perturbs * ppt,
                                 double k,
  ...
  /* Definitions */
  double mass=0, dmass=0, massold=0, massden=0, dmassden=0, massdenold=0, dtau=0;
  double a=0, H=0, Hp=0;
  double D=0, Dp=0, cs2n=0, cs2np=0;
  double g1=0, g1p=0, g2=0, g2p=0, g3=0, g3p=0;
  double trig=0, trig_extreme=0, dtaumin=0, dtaumax=0;
  double tau = tau_ini;
  int nswitch=0;
  
  /* Initialize the vector interval_number_smgqs. At the end of this routine
     it will contain some values of time useful to determine where the quasi-static
     approximation switches. The last value stores the sign of the denominator of the
     mass term */

  for (nswitch = 0; nswitch < (number_switches_accepted_smgqs + 1); nswitch++) {
    interval_number_smgqs[nswitch] = 0.;
  }
  nswitch = -1;

  /* Routine that calculates the times at which the quasi-static approximation switches */
  
  while ((tau < tau_end) && (nswitch < number_switches_accepted_smgqs) && (interval_number_smgqs[number_switches_accepted_smgqs] < 2.)) {
    

    /* Call the background_at_tau and stores */
    ...
    a = ppw->pvecback[pba->index_bg_a];
    H = ppw->pvecback[pba->index_bg_H];
    Hp = ppw->pvecback[pba->index_bg_H_prime];
    D = ppw->pvecback[pba->index_bg_kinetic_D_smg];
    Dp = ppw->pvecback[pba->index_bg_kinetic_D_prime_smg];
    cs2n = ppw->pvecback[pba->index_bg_cs2num_smg];
    cs2np = ppw->pvecback[pba->index_bg_cs2num_prime_smg];
    g1 = ppw->pvecback[pba->index_bg_gamma_1_smg];
    g1p = ppw->pvecback[pba->index_bg_gamma_1_prime_smg];
    g2 = ppw->pvecback[pba->index_bg_gamma_2_smg];
    g2p = ppw->pvecback[pba->index_bg_gamma_2_prime_smg];
    g3 = ppw->pvecback[pba->index_bg_gamma_3_smg];
    g3p = ppw->pvecback[pba->index_bg_gamma_3_prime_smg];
    trig = pow(ppr->smgqs_trigger_tau_over_tau_s,2);
    trig_extreme = pow(ppr->smgqs_extreme_trigger_tau_over_tau_s,2);
    dtaumin = ppr->smgqs_switch_step_min;
    dtaumax = ppr->smgqs_switch_step_max;
    
    
    //printf("gamma's = %e, %e, %e \n",g1,g2,g3);
//     printf("gamma_prime's = %e, %e, %e \n",g1p,g2p,g3p);

    /* Calculate the mass of the scalar field at tau */
    
    massden = D*pow(k,2) + g1*pow(H,2)*pow(a,2);
    
    dmassden = Dp*pow(k,2) + g1p*pow(H,2)*pow(a,2) + 2.*g1*H*Hp*pow(a,2) + 2.*g1*pow(H,3)*pow(a,3);

    mass = (cs2n*pow(k,4) + g3*pow(H,2)*pow(k,2)*pow(a,2) + g2*pow(H,4)*pow(a,4))*pow(H,-2)*pow(a,-2)*pow(massden,-1);
    
    dmass = ((g2*g1p + (-1.)*g1*g2p)*(-1.)*pow(H,7)*pow(a,6) + (cs2n*H*g1p + (-1.)*H*D*g3p + (-1.)*g1*H*cs2np + 4.*g1*cs2n*Hp + g3*H*Dp + 4.*g1*cs2n*pow(H,2)*a)*(-1.)*pow(H,2)*pow(k,4)*pow(a,2) + (g3*H*g1p + (-1.)*H*D*g2p + (-1.)*g1*H*g3p + 2.*g1*g3*Hp + (-2.)*g2*D*Hp + g2*H*Dp + 2.*g1*g3*pow(H,2)*a + (-2.)*g2*pow(H,2)*D*a)*(-1.)*pow(H,4)*pow(k,2)*pow(a,4) + (H*D*cs2np + (-2.)*cs2n*D*Hp + (-1.)*cs2n*H*Dp + (-2.)*cs2n*pow(H,2)*D*a)*pow(k,6))*pow(H,-3)*pow(a,-2)*pow(massden,-2);


    /* First time step: it decides if the quasi-static approximation should start from ON_extreme, ON or OFF. Additionally, it stores the sign of the denominaor of the mass term */

    if (nswitch == -1) {

      interval_number_smgqs[number_switches_accepted_smgqs] = 0.;
      massold = mass;
      massdenold = massden;

      if (mass/trig_extreme > 1.) {
        nswitch = 0;
      }
      else if (mass/trig > 1.) {
	nswitch = 1;
	interval_number_smgqs[nswitch-1] = tau_ini;
      }
      else {
	nswitch = 2;
	interval_number_smgqs[nswitch-2] = tau_ini;
	interval_number_smgqs[nswitch-1] = tau_ini;
      }
    }
    
    /* After the first step this routine stores only the values of tau where the quasi-static approximation switches. */
    
    if (interval_number_smgqs[number_switches_accepted_smgqs] == 0.) {

      if ((massden*massdenold > 0.) || ((mass != 0.) && (massold != 0.))) {

	massdenold = massden;
	
	if (((nswitch == 0) && ((mass/trig_extreme-1.)*(massold/trig_extreme-1.) < 0.)) || ((nswitch > 0) && ((mass/trig-1.)*(massold/trig-1.) < 0.))) {
          interval_number_smgqs[nswitch] = tau;
          nswitch += 1;
	}
      }
      else {
	...
	interval_number_smgqs[nswitch-1] = tau_ini;
	massdenold = massden;
	interval_number_smgqs[number_switches_accepted_smgqs] += 1;
      }
    }
    
    if (interval_number_smgqs[number_switches_accepted_smgqs] == 1.) {
      
      if ((massden*massdenold > 0.) || ((mass != 0.) && (massold != 0.))) {
	
	massdenold = massden;

	if ((nswitch == 1) && (mass > 0.) && (mass < trig)) {
          interval_number_smgqs[nswitch] = tau;
          nswitch += 1;
	}
	if ((nswitch > 1) && ((mass/trig-1.)*(massold/trig-1.) < 0.)) {
          interval_number_smgqs[nswitch] = tau;
          nswitch += 1;
	}
      }
    ...
    /* Update the value of the mass only if the point does not lie exactly on the 0 of the triggers */

    if ((mass/trig != 1.) && (mass/trig_extreme != 1.)) {
      massold = mass;
    }
    
    
    /* Choose the time step for the next iteration. If the step is too small or too large dtaumin or dtaumax will be used */
    

    dtau = MIN(abs(trig*(mass/trig-1.)/dmass), abs(trig_extreme*(mass/trig_extreme-1.)/dmass));
    dtau = MIN(dtau, abs(massden/dmassden));
    
    if (dtau < dtaumin) {
      tau += dtaumin;
    }
    else if (dtau > dtaumax) {
      tau += dtaumax;
    }
    else {
      tau += dtau;
  ...
  /* Fill the vector interval_number_smgqs with tau_end where the previous routine did not assigned any other value */
  
  for (nswitch = 0; nswitch < number_switches_accepted_smgqs; nswitch++) {
    if (interval_number_smgqs[nswitch] == 0.) {
      interval_number_smgqs[nswitch] = tau_end;
    }
  }
