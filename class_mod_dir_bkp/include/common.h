...
struct precision
{
  ...
  // TODO: rewrite the comments here
  
  /**
   * when to switch on the quasi-static approximation
   * for scalar-tensor theories
   * Condition: \f$ \tau_c/\tau_s \equiv  c_s k \tau \f$ >
   * quasi_static_trigger_tau_over_tau_s_smg.
   * Decrease this value to switch on earlier in time.
   * We also demand that recombination is over by some margin.
   */
  double smgqs_trigger_tau_over_tau_s;
  double smgqs_extreme_trigger_tau_over_tau_s;
  double smgqs_switch_step_min;
  double smgqs_switch_step_max;
  ...
  double l_switch_limber_for_cl_density;

  /** when to use the Limber approximation for density cl's (relative to central redshift of each bin) */
  double l_switch_limber_for_cl_lensing;
