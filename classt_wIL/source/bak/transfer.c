...
void print_transfer_info(struct transfers *ptr)
{
  ...
  fprintf(fo," selection_num_nc %d\n\n",ptr->selection_num_nc);
  for(ii=0;ii<ptr->selection_num_nc;ii++) {
    fprintf(fo," selection_mean_nc[%d]  %lf\n",ii,ptr->selection_mean_nc[ii]);
    fprintf(fo," selection_width_nc[%d] %lf\n",ii,ptr->selection_width_nc[ii]);
    fprintf(fo," selection_sz_nc[%d] %lf\n",ii,ptr->selection_sz_nc[ii]);
  }
  fprintf(fo,"\nDone\n");
  fclose(fo);
}
				  ...
				  )
{
  ...
  if(ppt->has_weak_lensing) {
	...
	free(glb_selection_0_wl[ii]);
	free(glb_selection_0_tau0_minus_tau_wl[ii]);
      }
      if(ppt->has_lensing_shear) {
	free(glb_selection_lns_wl[ii]);
	free(glb_selection_lns_tau0_minus_tau_wl[ii]);
      }
    }
    if(ppt->has_intrinsic_alignment) {
      free(glb_selection_0_wl);
      free(glb_selection_0_tau0_minus_tau_wl);
      free(glb_selection_0_size_wl);
    }
    if(ppt->has_lensing_shear) {
      free(glb_selection_lns_wl);
      free(glb_selection_lns_tau0_minus_tau_wl);
      free(glb_selection_lns_size_wl);
    }
  }
  ...
  if(ppt->has_cl_number_count) {
	...
	free(glb_selection_lns_tau0_minus_tau_nc[ii]);
      }
    }
    ...
    if((ppt->has_nc_lens) || (ppt->has_nc_gr4) || (ppt->has_nc_gr5)) {
      free(glb_selection_lns_size_nc);
      free(glb_selection_lns_tau0_minus_tau_nc);
    }
  }
}
...
int init_selection_functions_lensing_wl_dam(
  ...
  class_alloc(glb_selection_lns_wl,ptr->selection_num_wl*sizeof(double *),ptr->error_message);
  class_alloc(glb_selection_lns_tau0_minus_tau_wl,ptr->selection_num_wl*sizeof(double *),ptr->error_message);
  class_alloc(glb_selection_lns_size_wl,ptr->selection_num_wl*sizeof(int),ptr->error_message);
  ...
  for(ibin=0;ibin<ptr->selection_num_wl;ibin++) {
    ...
    glb_selection_lns_size_wl[ibin]=tau_size;
    class_alloc(glb_selection_lns_wl[ibin],tau_size*sizeof(double),ptr->error_message);
    class_alloc(glb_selection_lns_tau0_minus_tau_wl[ibin],tau_size*sizeof(double),ptr->error_message);
    ...
    class_call(transfer_lensing_sampling_wl(ppr,pba,ppt,ptr,ibin,tau0,
					    glb_selection_lns_tau0_minus_tau_wl[ibin],
					    glb_selection_lns_size_wl[ibin]),
	       ptr->error_message,ptr->error_message);
	...
	for(index_tau_sources=0;index_tau_sources<tau_sources_size;index_tau_sources++) {
	  if((tau0_minus_tau_lns_sources[index_tau_sources]>0.) &&
	     (tau0_minus_tau_lns_sources[index_tau_sources]-
	      glb_selection_lns_tau0_minus_tau_wl[ibin][index_tau]>0.)) {
	    rescaling_wl+=0.5*(glb_selection_lns_tau0_minus_tau_wl[ibin][index_tau]-
			       tau0_minus_tau_lns_sources[index_tau_sources])
	      /glb_selection_lns_tau0_minus_tau_wl[ibin][index_tau]
	      /tau0_minus_tau_lns_sources[index_tau_sources]
      ...
      glb_selection_lns_wl[ibin][index_tau]=rescaling_wl;
    }
  }
...
int init_selection_functions_lensing_nc_dam(
  ...
  if(ppt->has_nc_lens)
    class_alloc(glb_selection_lns_lns,ptr->selection_num_nc*sizeof(double *),ptr->error_message);
  if(ppt->has_nc_gr4)
    class_alloc(glb_selection_lns_gr4,ptr->selection_num_nc*sizeof(double *),ptr->error_message);
  if(ppt->has_nc_gr5)
    class_alloc(glb_selection_lns_gr5,ptr->selection_num_nc*sizeof(double *),ptr->error_message);
  class_alloc(glb_selection_lns_tau0_minus_tau_nc,ptr->selection_num_nc*sizeof(double *),ptr->error_message);
  class_alloc(glb_selection_lns_size_nc,ptr->selection_num_nc*sizeof(int),ptr->error_message);
  ...
  for(ibin=0;ibin<ptr->selection_num_nc;ibin++) {
    ...
    //Allocate and assign sizes
    glb_selection_lns_size_nc[ibin]=tau_size;
    if(ppt->has_nc_lens)
    ...
    class_alloc(glb_selection_lns_tau0_minus_tau_nc[ibin],tau_size*sizeof(double),ptr->error_message);
    ...
    class_call(transfer_lensing_sampling_nc(ppr,pba,ppt,ptr,ibin,tau0,
					 glb_selection_lns_tau0_minus_tau_nc[ibin],
					 glb_selection_lns_size_nc[ibin]),
	       ptr->error_message,ptr->error_message);
	...
	for(index_tau_sources=0;index_tau_sources<tau_sources_size;index_tau_sources++) {
	  if((tau0_minus_tau_lns_sources[index_tau_sources]>0.) &&
	     (tau0_minus_tau_lns_sources[index_tau_sources]-
	      glb_selection_lns_tau0_minus_tau_nc[ibin][index_tau]>0.)) {
	    double s_bias=array_bg[5*index_tau_sources+0];

	    if(ppt->has_nc_lens) {
	      rescaling_lns+=(2.-5.*s_bias)/2.
		*(glb_selection_lns_tau0_minus_tau_nc[ibin][index_tau]-
		  tau0_minus_tau_lns_sources[index_tau_sources])
		/glb_selection_lns_tau0_minus_tau_nc[ibin][index_tau]
		/tau0_minus_tau_lns_sources[index_tau_sources]
		*selection[index_tau_sources]
...
int init_selection_functions_nc_dam(
  ...
  class_alloc(pvecback,pba->bg_size*sizeof(double),ptr->error_message);
  class_alloc(glb_selection_0,ptr->selection_num_nc*sizeof(double *),ptr->error_message);
  class_alloc(glb_selection_0_tau0_minus_tau,ptr->selection_num_nc*sizeof(double *),ptr->error_message);
  class_alloc(glb_selection_0_size,ptr->selection_num_nc*sizeof(int),ptr->error_message);
  ...
  for(ibin=0;ibin<ptr->selection_num_nc;ibin++) {
    ...
    class_call(transfer_selection_sampling_nc(ppr,pba,ppt,ptr,ibin,glb_selection_0_tau0_minus_tau[ibin],tau_size),
	       ptr->error_message,ptr->error_message);
...
int init_selection_functions_wl_dam(
  ...
  class_alloc(pvecback,pba->bg_size*sizeof(double),ptr->error_message);
  class_alloc(glb_selection_0_wl,ptr->selection_num_wl*sizeof(double *),ptr->error_message);
  class_alloc(glb_selection_0_tau0_minus_tau_wl,ptr->selection_num_wl*sizeof(double *),ptr->error_message);
  class_alloc(glb_selection_0_size_wl,ptr->selection_num_wl*sizeof(int),ptr->error_message);
  ...
  for(ibin=0;ibin<ptr->selection_num_wl;ibin++) {
    ...
    glb_selection_0_size_wl[ibin]=tau_size;

    class_alloc(glb_selection_0_wl[ibin],tau_size*sizeof(double),ptr->error_message);
    class_alloc(glb_selection_0_tau0_minus_tau_wl[ibin],tau_size*sizeof(double),ptr->error_message);
    class_alloc(w_trapz,tau_size*sizeof(double),ptr->error_message);

    class_call(transfer_selection_sampling_wl(ppr,pba,ppt,ptr,ibin,glb_selection_0_tau0_minus_tau_wl[ibin],tau_size),
	       ptr->error_message,ptr->error_message);

    class_call(array_trapezoidal_mweights(glb_selection_0_tau0_minus_tau_wl[ibin],tau_size,
					  w_trapz,ptr->error_message),ptr->error_message,ptr->error_message);

    class_call(transfer_selection_compute_wl(ppr,pba,ppt,ptr,
					  glb_selection_0_wl[ibin],
					  glb_selection_0_tau0_minus_tau_wl[ibin],
					  w_trapz,tau_size,pvecback,tau0,ibin),
	       ptr->error_message,ptr->error_message);
...
int transfer_get_q_list(
  ...
  if (sgnK != 0) {
    ...
    if (ptr->transfer_verbose > 1)
      printf("Flat bessel approximation spares hyperspherical bessel computations for %zu "
  ...
  if (redefine_source == _TRUE_) {
	...
	itr=ptr->selection_tracer_nc[bin];
      ...
      if ((_index_tt_in_range_(ptr->index_tt_nc_lens, ptr->selection_num_nc, ppt->has_nc_lens)) ||
        ...
        /* redefine the time sampling */
	memcpy(tau0_minus_tau,glb_selection_lns_tau0_minus_tau_nc[bin],
	       glb_selection_lns_size_nc[bin]*sizeof(double));
	...
	class_alloc(pvecback,pba->bg_size*sizeof(double),ptr->error_message);
        ...
        /* redefine the time sampling */
	memcpy(tau0_minus_tau,glb_selection_0_tau0_minus_tau_wl[bin],
	       glb_selection_0_size_wl[bin]*sizeof(double));
        ...
        /* compute values of selection function at sampled values of tau */
	selection=glb_selection_0_wl[bin];
	...
	bin = index_tt - ptr->index_tt_lensing;
        ...
        /* redefine the time sampling */
	memcpy(tau0_minus_tau,glb_selection_lns_tau0_minus_tau_wl[bin],
	       glb_selection_lns_size_wl[bin]*sizeof(double));
        ...
        for (index_tau = 0; index_tau < tau_size; index_tau++) {
          /* lensing source =  - W(tau) (phi(k,tau) + psi(k,tau)) Heaviside(tau-tau_rec)
             ...
             regulated anyway by Bessel).
          */
	  rescaling=glb_selection_lns_wl[bin][index_tau];
...
int transfer_selection_function_nc(
  ...
  //  class_test((z<ptr->nz_z_nc[itr][0]) || (z>ptr->nz_z_nc[itr][ptr->nz_size_nc[itr]-1]),
  //	     ptr->error_message,
  //	     "Your input file for the selection function only covers the "
  //	     "redhsift range [%f : %f]. However, your input for the selection "
  //	     "function requires z=%f",
  //	     ptr->nz_z_nc[itr][0],
  //	     ptr->nz_z_nc[itr][ptr->nz_size_nc[itr]-1],
  //	     z);
  if((z<ptr->nz_z_nc[itr][0]) || (z>ptr->nz_z_nc[itr][ptr->nz_size_nc[itr]-1])) {
    //    printf(" Warning: extrapolating selection function outside redshift range to 0\n");
    *selection=0.;
    return _SUCCESS_;
  }
...
int transfer_selection_function_wl(
  ...
  //  class_test((z<ptr->nz_z_wl[itr][0]) || (z>ptr->nz_z_wl[itr][ptr->nz_size_wl[itr]-1]),
  //	     ptr->error_message,
  //	     "Your input file for the selection function only covers the "
  //	     "redhsift range [%f : %f]. However, your input for the selection "
  //	     "function requires z=%f",
  //	     ptr->nz_z_wl[itr][0],
  //	     ptr->nz_z_wl[itr][ptr->nz_size_wl[itr]-1],
  //	     z);
  ...
  if (radial_type == SCALAR_TEMPERATURE_0) {
    ...
    *trsf =
    ...
    class_call(transfer_limber_interpolate(ptr,
               ...
               ptr->error_message);
    *trsf =
      sqrt(_PI_/(2.*l+3.))*Sp/(l+1.5)*4/(2*l+3)-
      sqrt(_PI_/(2.*l+1.))*Sm/(l+0.5)*(1.+8.*l)/((2.*l+1.)*(2.*l+1.));
    /* DAM: I think this is a bug
...
int transfer_select_radial_function(
  ...
  /* generic case leading to generic bessel function (it applies also to all 
     nonCMB types: lcmb, density, lensing) */
