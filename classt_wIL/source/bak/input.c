  ...
  if (argc > 1) {
    for (i=1; i<argc; i++) {
      ...
      if (strcmp(extension,".ini") == 0) {
        ...
        fprintf(stdout,"Warning: the file %s has an extension different from .ini and .pre, so "
		"it has been ignored\n",argv[i]);
      }
    }
  }
...
int input_read_parameters(
  ...
  class_read_double("wl_epsilon",pba->wl_epsilon);
...
int input_default_precision ( struct precision * ppr ) {
  ...
  ppr->l_switch_limber_for_cl_density=100.;
  ppr->l_switch_limber_for_cl_lensing=100.;
