...
int perturb_init(
  ...
  class_test((ppt->gauge == synchronous) && (pba->has_cdm == _FALSE_),
             ppt->error_message,
             "In the synchronous gauge, it is not self-consistent to assume no CDM: the later is used to "
	     "define the initial timelike hypersurface. You can either add a negligible amount of CDM or "
	     "switch to newtonian gauge");

  class_test ((ppr->tight_coupling_approximation < first_order_MB) ||
              (ppr->tight_coupling_approximation > compromise_CLASS),
              ppt->error_message,
              "your tight_coupling_approximation is set to %d, out of range defined in perturbations.h",
  ...
  class_test ((ppr->radiation_streaming_approximation < rsa_null) ||
              ...
              ppt->error_message,
              "your radiation_streaming_approximation is set to %d, out of range defined in perturbations.h",
  ...
  if (pba->has_ncdm == _TRUE_) {
    ...
    class_test ((ppr->ncdm_fluid_approximation < ncdmfa_mb) ||
                ...
                ppt->error_message,
                "your ncdm_fluid_approximation is set to %d, out of range defined in perturbations.h",
  ...
  if ((ppt->has_niv == _TRUE_) && (ppt->perturbations_verbose > 0)) {
    printf("Warning: the niv initial conditions in CLASS (and also in CAMB) should still be "
...
#pragma omp for schedule (dynamic)
        ...
        for (index_k = ppt->k_size[index_md]-1; index_k >=0; index_k--) {
          ...
          if ((ppt->perturbations_verbose > 2) && (abort == _FALSE_)) {
            printf("evolving mode k=%e /Mpc  (%d/%d)",ppt->k[index_md][index_k],
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_tensors_) {
          ...
          if ((interval_approx[index_switch-1][ppw->index_ap_rsa]==(int)rsa_off) &&
              (interval_approx[index_switch][ppw->index_ap_rsa]==(int)rsa_on))
            fprintf(stdout,"Mode k=%e: will switch on radiation streaming approximation for tensors "
		    "at tau=%e\n",k,interval_limit[index_switch]);
