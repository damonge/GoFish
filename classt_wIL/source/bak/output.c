...
FILE *fopen_protected(char *fname,char *mode)
{
...
static int output_write_cls_binary(struct spectra * psp,
				   struct output * pop)
{
  char fname[256],prefix[256];
  FILE *fo;
  int il,ii,index_d1,index_d2,index_cl;
  double **dl_grid;
  double *cl;

  sprintf(prefix,"%s_cl",pop->root);

  //Allocate table for power spectra
  cl=malloc(psp->ct_size*sizeof(double *));
  if(cl==NULL)
    my_abort(1,"Out of memory\n");

  dl_grid=malloc(psp->ct_size*sizeof(double *));
  if(dl_grid==NULL)
    my_abort(1,"Out of memory\n");
  for(ii=0;ii<psp->ct_size;ii++) {
    dl_grid[ii]=malloc((psp->l_max_tot-1)*sizeof(double));
    if(dl_grid[ii]==NULL)
      my_abort(1,"Out of memory\n");
  }

  for(il=0;il<psp->l_max_tot-1;il++) {
    double l=(double)(il+2);
    double factor=l*(l+1)/2./_PI_;
    class_call(spectra_cl_at_l(psp,l,cl,NULL,NULL),
	       psp->error_message,pop->error_message);
    for(ii=0;ii<psp->ct_size;ii++)
      dl_grid[ii][il]=factor*cl[ii];
  }
  free(cl);

  //First write CMB-only power spectra
  if(psp->has_tt==_TRUE_) { //Write T x T
    sprintf(fname,"%s_tt.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(dl_grid[psp->index_ct_tt],sizeof(double),psp->l_max_tot-1,fo);
    fclose(fo);
  }	
  if(psp->has_ee==_TRUE_) { //Write E x E
    sprintf(fname,"%s_ee.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(dl_grid[psp->index_ct_ee],sizeof(double),psp->l_max_tot-1,fo);
    fclose(fo);
  }	
  if(psp->has_te==_TRUE_) { //Write T x E
    sprintf(fname,"%s_te.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(dl_grid[psp->index_ct_te],sizeof(double),psp->l_max_tot-1,fo);
    fclose(fo);
  }	
  if(psp->has_bb==_TRUE_) { //Write B x B
    sprintf(fname,"%s_bb.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(dl_grid[psp->index_ct_bb],sizeof(double),psp->l_max_tot-1,fo);
    fclose(fo);
  }	
  if(psp->has_pp==_TRUE_) { //Write CL x CL
    sprintf(fname,"%s_pp.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(dl_grid[psp->index_ct_pp],sizeof(double),psp->l_max_tot-1,fo);
    fclose(fo);
  }	
  if(psp->has_tp==_TRUE_) { //Write T x CL
    sprintf(fname,"%s_tp.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(dl_grid[psp->index_ct_tp],sizeof(double),psp->l_max_tot-1,fo);
    fclose(fo);
  }	
  if(psp->has_ep==_TRUE_) { //Write E x CL
    sprintf(fname,"%s_ep.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(dl_grid[psp->index_ct_ep],sizeof(double),psp->l_max_tot-1,fo);
    fclose(fo);
  }	

  //Spectra containing number counts or lensing only
  if(psp->has_dd==_TRUE_) { //NC x NC
    sprintf(fname,"%s_dd.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(&(psp->d_size_nc),sizeof(int),1,fo);
    
    index_cl=0;
    for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
      for(index_d2=index_d1;index_d2<psp->d_size_nc;index_d2++) {
	my_fwrite(&(index_d1),sizeof(int),1,fo);
	my_fwrite(&(index_d2),sizeof(int),1,fo);
	my_fwrite(dl_grid[psp->index_ct_dd+index_cl],sizeof(double),psp->l_max_tot-1,fo);
	index_cl++;
      }
    }
    fclose(fo);
  }
  if(psp->has_ll==_TRUE_) { //L x L
    sprintf(fname,"%s_ll.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(&(psp->d_size_wl),sizeof(int),1,fo);
	...
	my_fwrite(&(index_d1),sizeof(int),1,fo);
	my_fwrite(&(index_d2),sizeof(int),1,fo);
	my_fwrite(dl_grid[psp->index_ct_ll+index_cl],sizeof(double),psp->l_max_tot-1,fo);
	index_cl++;
      }
    }
    fclose(fo);
  }
  if(psp->has_dl==_TRUE_) { //NC x L
    sprintf(fname,"%s_dl.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(&(psp->d_size_nc),sizeof(int),1,fo);
    my_fwrite(&(psp->d_size_wl),sizeof(int),1,fo);
	...
	my_fwrite(&(index_d1),sizeof(int),1,fo);
	my_fwrite(&(index_d2),sizeof(int),1,fo);
	my_fwrite(dl_grid[psp->index_ct_dl+index_cl],sizeof(double),psp->l_max_tot-1,fo);
	index_cl++;
      }
    }
    fclose(fo);
  ...
  if(psp->has_td==_TRUE_) { //T x NC
    sprintf(fname,"%s_td.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(&(psp->d_size_nc),sizeof(int),1,fo);
    ...
    for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
      my_fwrite(&(index_d1),sizeof(int),1,fo);
      my_fwrite(dl_grid[psp->index_ct_td+index_d1],sizeof(double),psp->l_max_tot-1,fo);
    }
    fclose(fo);
  }
  if(psp->has_pd==_TRUE_) { //CL x NC
    sprintf(fname,"%s_pd.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(&(psp->d_size_nc),sizeof(int),1,fo);
    ...
    for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
      my_fwrite(&(index_d1),sizeof(int),1,fo);
      my_fwrite(dl_grid[psp->index_ct_pd+index_d1],sizeof(double),psp->l_max_tot-1,fo);
    }
    fclose(fo);
  }
  if(psp->has_pl==_TRUE_) { //CL x L
    sprintf(fname,"%s_pl.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(&(psp->d_size_wl),sizeof(int),1,fo);
    ...
    for(index_d1=0;index_d1<psp->d_size_wl;index_d1++) {
      my_fwrite(&(index_d1),sizeof(int),1,fo);
      my_fwrite(dl_grid[psp->index_ct_pl+index_d1],sizeof(double),psp->l_max_tot-1,fo);
    }
    fclose(fo);
  }
  if(psp->has_tl==_TRUE_) { //T x L
    sprintf(fname,"%s_tl.dat",prefix);
    fo=fopen_protected(fname,"wb");
    my_fwrite(&(psp->l_max_tot),sizeof(int),1,fo);
    my_fwrite(&(psp->d_size_wl),sizeof(int),1,fo);
    ...
    for(index_d1=0;index_d1<psp->d_size_wl;index_d1++) {
      my_fwrite(&(index_d1),sizeof(int),1,fo);
      my_fwrite(dl_grid[psp->index_ct_tl+index_d1],sizeof(double),psp->l_max_tot-1,fo);
    }
    fclose(fo);
  }
...
static int output_write_cls_ascii(struct spectra * psp,
				  struct output * pop)
{
  char fname[256],prefix[256];
  FILE *fo;
  ...
  //First write CMB-only power spectra
  if(psp->has_tt==_TRUE_) { //Write T x T
    sprintf(fname,"%s_tt.txt",prefix);
    fo=fopen_protected(fname,"w");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    fprintf(fo,"# [1] TT\n");
    for(il=0;il<psp->l_max_tot-1;il++)
      fprintf(fo,"%d %lE\n",il+2,dl_grid[psp->index_ct_tt][il]);
    fclose(fo);
  }	
  if(psp->has_ee==_TRUE_) { //Write E x E
    sprintf(fname,"%s_ee.txt",prefix);
    fo=fopen_protected(fname,"w");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    fprintf(fo,"# [1] EE\n");
    for(il=0;il<psp->l_max_tot-1;il++)
      fprintf(fo,"%d %lE\n",il+2,dl_grid[psp->index_ct_ee][il]);
    fclose(fo);
  }	
  if(psp->has_te==_TRUE_) { //Write T x E
    sprintf(fname,"%s_te.txt",prefix);
    fo=fopen_protected(fname,"w");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    fprintf(fo,"# [1] TE\n");
    for(il=0;il<psp->l_max_tot-1;il++)
      fprintf(fo,"%d %lE\n",il+2,dl_grid[psp->index_ct_te][il]);
    fclose(fo);
  }	
  if(psp->has_bb==_TRUE_) { //Write B x B
    sprintf(fname,"%s_bb.txt",prefix);
    fo=fopen_protected(fname,"w");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    fprintf(fo,"# [1] BB\n");
    for(il=0;il<psp->l_max_tot-1;il++)
      fprintf(fo,"%d %lE\n",il+2,dl_grid[psp->index_ct_bb][il]);
    fclose(fo);
  }	
  if(psp->has_pp==_TRUE_) { //Write CL x CL
    sprintf(fname,"%s_pp.txt",prefix);
    fo=fopen_protected(fname,"w");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    fprintf(fo,"# [1] PP\n");
    for(il=0;il<psp->l_max_tot-1;il++)
      fprintf(fo,"%d %lE\n",il+2,dl_grid[psp->index_ct_pp][il]);
    fclose(fo);
  }	
  if(psp->has_tp==_TRUE_) { //Write T x CL
    sprintf(fname,"%s_tp.txt",prefix);
    fo=fopen_protected(fname,"w");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    fprintf(fo,"# [1] TP\n");
    for(il=0;il<psp->l_max_tot-1;il++)
      fprintf(fo,"%d %lE\n",il+2,dl_grid[psp->index_ct_tp][il]);
    fclose(fo);
  }	
  if(psp->has_ep==_TRUE_) { //Write E x CL
    sprintf(fname,"%s_ep.txt",prefix);
    fo=fopen_protected(fname,"w");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    fprintf(fo,"# [1] EP\n");
    for(il=0;il<psp->l_max_tot-1;il++)
      fprintf(fo,"%d %lE\n",il+2,dl_grid[psp->index_ct_ep][il]);
    fclose(fo);
  }	

  //Spectra containing number counts or lensing only
  if(psp->has_dd==_TRUE_) { //NC x NC
    sprintf(fname,"%s_dd.txt",prefix);
    fo=fopen_protected(fname,"wb");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    index_cl=0;
    for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
      for(index_d2=index_d1;index_d2<psp->d_size_nc;index_d2++) {
	fprintf(fo," [%d] d%dd%d,",index_cl+1,index_d1+1,index_d2+1);
	index_cl++;
      }
    }
    fprintf(fo,"\n");
    for(il=0;il<psp->l_max_tot-1;il++) {
      ...
      for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
	for(index_d2=index_d1;index_d2<psp->d_size_nc;index_d2++) {
	  fprintf(fo,"%lE ",dl_grid[psp->index_ct_dd+index_cl][il]);
	  index_cl++;
	}
      }
      fprintf(fo,"\n");
  ...
  if(psp->has_ll==_TRUE_) { //L x L
    ...
    for(index_d1=0;index_d1<psp->d_size_wl;index_d1++) {
      for(index_d2=index_d1;index_d2<psp->d_size_wl;index_d2++) {
	fprintf(fo," [%d] l%dl%d,",index_cl+1,index_d1+1,index_d2+1);
	index_cl++;
      }
    }
    fprintf(fo,"\n");
    for(il=0;il<psp->l_max_tot-1;il++) {
      ...
      for(index_d1=0;index_d1<psp->d_size_wl;index_d1++) {
	for(index_d2=index_d1;index_d2<psp->d_size_wl;index_d2++) {
	  fprintf(fo,"%lE ",dl_grid[psp->index_ct_ll+index_cl][il]);
	  index_cl++;
	}
      }
      fprintf(fo,"\n");
  ...
  if(psp->has_dl==_TRUE_) { //L x L
    sprintf(fname,"%s_dl.txt",prefix);
    fo=fopen_protected(fname,"wb");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    index_cl=0;
    for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
      for(index_d2=0;index_d2<psp->d_size_wl;index_d2++) {
	fprintf(fo," [%d] d%dl%d,",index_cl+1,index_d1+1,index_d2+1);
	index_cl++;
      }
    }
    fprintf(fo,"\n");
    for(il=0;il<psp->l_max_tot-1;il++) {
      ...
      for(index_d1=0;index_d1<psp->d_size_nc;index_d1++) {
	for(index_d2=0;index_d2<psp->d_size_wl;index_d2++) {
	  fprintf(fo,"%lE ",dl_grid[psp->index_ct_dl+index_cl][il]);
	  index_cl++;
	}
      }
      fprintf(fo,"\n");
  ...
  //Cross-correlation with CMB
  if(psp->has_td==_TRUE_) { //T x NC
    sprintf(fname,"%s_td.txt",prefix);
    fo=fopen_protected(fname,"wb");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    for(index_d1=0;index_d1<psp->d_size_nc;index_d1++)
      fprintf(fo," [%d] td%d,",index_d1+1,index_d1+1);
    fprintf(fo,"\n");
    for(il=0;il<psp->l_max_tot-1;il++) {
      fprintf(fo,"%d ",il+2);
      for(index_d1=0;index_d1<psp->d_size_nc;index_d1++)
	fprintf(fo,"%lE ",dl_grid[psp->index_ct_td+index_d1][il]);
      fprintf(fo,"\n");
    }
    fclose(fo);
  }
  if(psp->has_pd==_TRUE_) { //CL x NC
    sprintf(fname,"%s_pd.txt",prefix);
    fo=fopen_protected(fname,"wb");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    for(index_d1=0;index_d1<psp->d_size_nc;index_d1++)
      fprintf(fo," [%d] pd%d,",index_d1+1,index_d1+1);
    fprintf(fo,"\n");
    for(il=0;il<psp->l_max_tot-1;il++) {
      fprintf(fo,"%d ",il+2);
      for(index_d1=0;index_d1<psp->d_size_nc;index_d1++)
	fprintf(fo,"%lE ",dl_grid[psp->index_ct_pd+index_d1][il]);
      fprintf(fo,"\n");
    }
    fclose(fo);
  }
  if(psp->has_pl==_TRUE_) { //CL x L
    sprintf(fname,"%s_pl.txt",prefix);
    fo=fopen_protected(fname,"wb");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    for(index_d1=0;index_d1<psp->d_size_wl;index_d1++)
      fprintf(fo," [%d] pl%d,",index_d1+1,index_d1+1);
    fprintf(fo,"\n");
    for(il=0;il<psp->l_max_tot-1;il++) {
      fprintf(fo,"%d ",il+2);
      for(index_d1=0;index_d1<psp->d_size_wl;index_d1++)
	fprintf(fo,"%lE ",dl_grid[psp->index_ct_pl+index_d1][il]);
      fprintf(fo,"\n");
    }
    fclose(fo);
  }
  if(psp->has_tl==_TRUE_) { //CL x NC
    sprintf(fname,"%s_tl.txt",prefix);
    fo=fopen_protected(fname,"wb");
    fprintf(fo,"# Dimensionless power spectra l*(l+1)*C_l/(2*pi)\n");
    fprintf(fo,"# [0] l,");
    for(index_d1=0;index_d1<psp->d_size_wl;index_d1++)
      fprintf(fo," [%d] tl%d,",index_d1+1,index_d1+1);
    fprintf(fo,"\n");
    for(il=0;il<psp->l_max_tot-1;il++) {
      fprintf(fo,"%d ",il+2);
      for(index_d1=0;index_d1<psp->d_size_wl;index_d1++)
	fprintf(fo,"%lE ",dl_grid[psp->index_ct_tl+index_d1][il]);
      fprintf(fo,"\n");
    }
    fclose(fo);
  }
...
int output_write_cls_classt(struct spectra * psp,
			    struct output * pop)
{
  if(pop->write_binary==1)
    return output_write_cls_binary(psp,pop);
  else
    return output_write_cls_ascii(psp,pop);
}
...
#ifdef _CLASST
    output_write_cls_classt(psp,pop);
#else //_CLASST
...
int output_open_cl_file(
  ...
  if (pop->write_header == _TRUE_) {
    ...
    if (psp->has_pl == _TRUE_){
      for (index_d1=0; index_d1<psp->d_size_wl; index_d1++){
        sprintf(tmp,"phi-lens[%d]",index_d1+1);
        class_fprintf_columntitle(*clfile,tmp,_TRUE_,colnum);
      }
    }
