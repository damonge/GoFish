...
int spectra_cls(
  ...
  double * cl_integrand; /* array withargument cl_integrand[index_k*cl_integrand_num_columns+1+psp->index_ct] */
  ...
  for (index_md = 0; index_md < psp->md_size; index_md++) {
    ...
    class_alloc(psp->cl[index_md],sizeof(double)*psp->l_size[index_md]*
		psp->ct_size*psp->ic_ic_size[index_md],psp->error_message);
    class_alloc(psp->ddcl[index_md],sizeof(double)*psp->l_size[index_md]*
		psp->ct_size*psp->ic_ic_size[index_md],psp->error_message);
...
#pragma omp flush(abort)
            ...
            if (psp->spectra_verbose > 1)
              printf("In %s: time spent in parallel region (loop over l's) "
		     "= %e s for thread %d\n",
...
int spectra_compute_cl(
  ...
  for (index_q=0; index_q < ptr->q_size; index_q++) {
    ...
    if (ppt->has_cl_cmb_temperature == _TRUE_) {
      ...
      if (_scalars_) {
        transfer_ic1_temp=transfer_ic1[ptr->index_tt_t0]+transfer_ic1[ptr->index_tt_t1]+
	  transfer_ic1[ptr->index_tt_t2];
        transfer_ic2_temp=transfer_ic2[ptr->index_tt_t0]+transfer_ic2[ptr->index_tt_t1]+
	...
	if (ppt->has_nc_rsd2    ==_TRUE_) {
          transfer_ic1_nc[index_d1] +=greps*transfer_ic1[ptr->index_tt_d0+index_d1];
          transfer_ic2_nc[index_d1] +=greps*transfer_ic2[ptr->index_tt_d0+index_d1];
	}
	if (ppt->has_nc_rsd3    ==_TRUE_) {
          transfer_ic1_nc[index_d1] +=greps*transfer_ic1[ptr->index_tt_d1+index_d1];
          transfer_ic2_nc[index_d1] +=greps*transfer_ic2[ptr->index_tt_d1+index_d1];
	}

        if (ppt->has_nc_lens == _TRUE_) {
          transfer_ic1_nc[index_d1] +=
            ell*(ell+1)*wleps*transfer_ic1[ptr->index_tt_nc_lens+index_d1];
          transfer_ic2_nc[index_d1] +=
            ell*(ell+1)*wleps*transfer_ic2[ptr->index_tt_nc_lens+index_d1];
        }
	    ...
	    greps*transfer_ic1[ptr->index_tt_nc_g1+index_d1];
          transfer_ic2_nc[index_d1] +=
	    greps*transfer_ic2[ptr->index_tt_nc_g1+index_d1];
	}
        if (ppt->has_nc_gr2 == _TRUE_) {
          transfer_ic1_nc[index_d1] +=
	    greps*transfer_ic1[ptr->index_tt_nc_g2+index_d1];
          transfer_ic2_nc[index_d1] +=
	    greps*transfer_ic2[ptr->index_tt_nc_g2+index_d1];
	}
        if (ppt->has_nc_gr3 == _TRUE_) {
          transfer_ic1_nc[index_d1] +=
	    greps*transfer_ic1[ptr->index_tt_nc_g3+index_d1];
          transfer_ic2_nc[index_d1] +=
	    greps*transfer_ic2[ptr->index_tt_nc_g3+index_d1];
	}
        if (ppt->has_nc_gr4 == _TRUE_) {
          transfer_ic1_nc[index_d1] +=
	    greps*transfer_ic1[ptr->index_tt_nc_g4+index_d1];
          transfer_ic2_nc[index_d1] +=
	    greps*transfer_ic2[ptr->index_tt_nc_g4+index_d1];
	}
        if (ppt->has_nc_gr5 == _TRUE_) {
          transfer_ic1_nc[index_d1] +=
	    greps*transfer_ic1[ptr->index_tt_nc_g5+index_d1];
          transfer_ic2_nc[index_d1] +=
...
int spectra_pk(
  ...
  if (psp->spectra_verbose>0)
    fprintf(stdout," -> sigma8=%g (computed till k = %g h/Mpc)\n",
	    psp->sigma8,exp(psp->ln_k[psp->ln_k_size-1])/pba->h);
